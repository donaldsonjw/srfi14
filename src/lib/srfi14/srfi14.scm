...
(module srfi14
   (export
      ...
      (make-char-set s::bstring)
      (char-set? v)
      (char-set:s char-set::<char-set>)
      (char-set-copy cs)
      (char-set= . rest)
      (char-set<= . rest)
      (char-set-hash cs . maybe-bound)
      (char-set-contains? cs char)
      (char-set-size cs)
      (char-set-count pred cset)
      (char-set-adjoin cs . chars)
      (char-set-adjoin! cs . chars)
      (char-set-delete cs . chars)
      (char-set-delete! cs . chars)
      ...
      (char-set-unfold p f g seed . maybe-base)
      (char-set-unfold! p f g seed base-cset)
      (char-set . chars)
      (list->char-set chars . maybe-base)
      (list->char-set! chars base-cs)
      (char-set->list cs)
      (string->char-set str . maybe-base)
      (string->char-set! str base-cs)
      (char-set->string cs)
      (ucs-range->char-set lower upper . rest)
      (ucs-range->char-set! lower upper error? base-cs)
      (char-set-filter predicate domain . maybe-base)
      (char-set-filter! predicate domain base-cs)
      (->char-set x)
      (char-set-complement cs)
      (char-set-complement! cset)
      (char-set-union! cset1 . csets)
      (char-set-union . csets)
      (char-set-intersection! cset1 . csets)
      (char-set-intersection . csets)
      (char-set-difference! cset1 . csets)
      (char-set-difference cs1 . csets)
      (char-set-xor! cset1 . csets)
      (char-set-xor . csets)
      (char-set-diff+intersection! cs1 cs2 . csets)
      (char-set-diff+intersection cs1 . csets)
      char-set:empty
      char-set:full
      char-set:lower-case
      char-set:upper-case
...
(define (srfi14-error msg . args)
   (error "srfi14" msg args))
             ...
             (e `(cond-expand
                    (bigloo-debug
                     (let ((,vsym ,val))
                        (if (,pred ,vsym)
                            ,vsym
                            (srfi14-error ',proc "Bad arg" ,vsym))))
                    (else
                     ,val)) e)))
         (else
...
;; original define-record definition of char-set
; (define-record-type char-set
;   (make-char-set s)
;   char-set?
;   (s char-set:s))

;; For Bigloo we implement char-set as a class and define
;; the following constructor accessor procedures 

(define (make-char-set s::bstring)
   (instantiate::<char-set> (s s)))
(define (char-set? v)
   (isa? v <char-set>))
(define (char-set:s char-set::<char-set>)
   (-> char-set s))
...
(define (%default-base maybe-base proc)
  (if (pair? maybe-base)
	...
	(if (null? tail)
		...
		(srfi14-error "BASE-CS parameter not a char-set" proc bcs))
	    (srfi14-error "Expected final base char set -- too many parameters"
		   proc maybe-base)))
...
(define (%char-set:s/check cs proc)
  (let lp ((cs cs))
	...
	(lp (srfi14-error "Not a char-set" cs proc)))))
...
(define (char-set-hash cs . maybe-bound)
  (let* ((bound (srfi14-optional maybe-bound 4194304 (lambda (n) (and (integer? n)
								(exact? n)
								(<= 0 n)))))
...
(define (%ucs-range->char-set! lower upper error? bs proc)
  ...
  (if (and (< lower upper) (< 256 upper) error?)
      (srfi14-error "Requested UCS range contains unavailable characters -- this implementation only supports Latin-1"
	     proc lower upper))
...
(define (->char-set x)
  (cond ((char-set? x) x)
	...
	((char? x) (char-set x))
	(else (srfi14-error "->char-set: Not a charset, string or char." x))))
